...
class ListWordService{
    companion object : WordService {
        ...
        init {
            ...
            val word = Word(1, "désultoire", "Du latin desultorius," +
            ...
            val word1 = Word(2,"Habitudinaire", "Théol., celui qui commet les mêmes péchés"
                    , descList1)
            ...
            val word2 = Word(3,"Ophite", "Membre d'une secte gnostique du IIème siècle qui vouait un culte au serpent et voyait en cet animal un symbole du messie"
                    , descList2)
            ...
            val word3 = Word(4,"Paraschiste", "Embaumeur de cadavre spécialisé dans l'extraction des vicères"
                    , descList3)
            datas = linkedMapOf(word.id to word, 2 to word1, 3 to word2, 4 to word3)
            //create the datas
        }

        override fun getWords(): ArrayList<Word>{
            return ArrayList(datas.values)
        }
        ...
        fun getRandomWord() :Word{
            val wordsAsArray = ArrayList(datas.values)
            val r = Random()
